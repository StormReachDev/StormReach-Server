...
export const signIn = asyncWrapper(async function (req, res, next) {
  ...
  if (!email || !password) {
    return next(
      new ErrorHandler(
        'Oops! Looks like you missed entering your email or password.',
        400,
      ),
    );
  }
...
export const forgotPassword = asyncWrapper(async function (req, res, next) {
  ...
  if (!email) {
    return next(
      new ErrorHandler('Oops! Looks like you missed entering your email.', 400),
    );
  }
...
export const resetPassword = asyncWrapper(async function (req, res, next) {
  ...
  if (!password || !confirmPassword) {
    return next(
      new ErrorHandler(
        '"Don’t forget to fill out both your password and confirm password.',
        400,
      ),
    );
  }
